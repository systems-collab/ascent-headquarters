...
export async function persistQuestionnaireIfNeeded(): Promise<boolean> {
  ...
  // Upsert profile by id so we don't silently no-op if the auth trigger hasn't
  // landed yet. full_name is required by the schema, so fall back to email.
  const { error: profileError } = await supabase.from("profiles").upsert(
    {
      id: user.id,
      full_name: formData.full_name || user.email || "Founder",
  ...
  const { error: startupError } = await supabase.from("startups").upsert(
    {
      founder_id: user.id,
      name: formData.startup_name || "Untitled startup",
  ...
  const { error: rpcError } = await supabase.rpc("compute_readiness_score", {
